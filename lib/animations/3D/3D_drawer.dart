...
class ThreeDDrawer extends StatefulWidget {
  final Widget child;
  final Widget drawer;
  const ThreeDDrawer({Key? key, required this.child, required this.drawer})
      : super(key: key);
...
class _ThreeDDrawerState extends State<ThreeDDrawer>
  ...
  void initState() {
    ...
    _yRotationAnimationForChild =
        Tween<double>(begin: 0.0, end: -pi / 2).animate(_xControllerForChild);
    ...
    _yRotationAnimationForDrawer =
        Tween<double>(begin: -pi / 2.7, end: 0.0).animate(_xControllerForDrawer);
  }
